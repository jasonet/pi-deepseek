 ...
 * pi-mcp-unity
 *
 * Bridges the CoplayDev open-source Unity MCP server into pi as native tools.
 * It reads its own mcp.json, connects to each configured Unity MCP server
 * (stdio via `uvx`), discovers their tools, and registers every remote tool as
 * a native pi tool named `<server>__<tool>` (e.g. `coplaydev_unity__*`).
 *
 * Requirements:
 *   - `uvx` on PATH (uv / pipx); the extension augments PATH for GUI launches.
 *   - The MCPForUnity package installed in your Unity project, and
 *     "Start Server" pressed in the Unity → MCP window (listens on 127.0.0.1).
 *
 * Commands:
 *   /unity-mcp            Show connection + tool status, `reconnect` to retry.
 *   /concept-to-scene ... Concept → Scene closed loop (Higgsfield art + Unity).
 */
...
const EXT_ID = "pi-mcp-unity";
const STATUS_KEY = "mcp-unity";
const STATUS_LABEL = "Unity MCP";
...
const CONFIG_PATH =
  process.env.PI_MCP_UNITY_CONFIG ||
 ...
 * child processes. A GUI-launched host can inherit a stripped PATH where `uvx`
 * is missing; without this the Unity MCP server fails to spawn with ENOENT.
 */
...
export default async function (pi: ExtensionAPI) {
  ...
  // Connect in the background so a slow first-run `uvx` download never blocks
  // extension load / app startup. Tools register as the server connects.
  ...
  pi.registerCommand("unity-mcp", {
    description: "Show Unity MCP status and reconnect servers",
    handler: async (args, ctx) => {
      if (args.trim() === "reconnect") {
        ctx.ui.notify("Reconnecting Unity MCP...", "info");
        await connectAll((m, l) => ctx.ui.notify(m, l === "warn" ? "warn" : l));
      }
      ctx.ui.notify(statusText(), "info");
    },
    getArgumentCompletions: (prefix: string) => {
      const items = [{ value: "reconnect", label: "reconnect — close & reconnect Unity MCP" }];
...
- Unity 工具，名字形如 \`coplaydev_unity__*\`（操作 Unity 编辑器：生成/导入资产、建场景、相机截图等）。
...
2. **入引擎**：用 \`coplaydev_unity__*\` 工具把该资产导入并应用到场景（贴到材质 / 设为天空盒 / 铺到地表，取决于资产类型）。
3. **看效果**：用 \`coplaydev_unity__*\` 的场景截图工具（如 SceneView/Camera capture）截一张当前场景图。
...
约束：
- Higgsfield 产的是 2D 图/视频，不是 3D 网格；3D 网格请用 \`coplaydev_unity__*\` 的资产生成工具。
